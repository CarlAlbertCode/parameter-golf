...
class Hyperparameters:
    ...
    low_rank_mlp_rank = int(os.environ.get('LOW_RANK_MLP_RANK', 0))
    ...
    progressive_looping_enabled = bool(int(os.environ.get('PROGRESSIVE_LOOPING_ENABLED', '1')))
    loop_stage1_at = float(os.environ.get('LOOP_STAGE1_AT', 0.28))
    loop_stage1_loops = int(os.environ.get('LOOP_STAGE1_LOOPS', 1))
    loop_stage2_at = float(os.environ.get('LOOP_STAGE2_AT', 0.35))
...
class FactorizedLinear(nn.Module):
    ...
    def __init__(self, in_features, out_features, rank, bias=False, zero_init=False):
        super().__init__()
        if rank <= 0:
            raise ValueError(f'rank must be positive, got {rank}')
        if rank >= min(in_features, out_features):
            raise ValueError(f'rank={rank} must be smaller than min(in_features, out_features)={min(in_features, out_features)}')
        self.in_features = in_features
        self.out_features = out_features
        self.rank = rank
        self.v = CastedLinear(in_features, rank, bias=False)
        self.u = CastedLinear(rank, out_features, bias=bias)
        if zero_init:
            self.u._zero_init = True
...
class MLP(nn.Module):
    ...
    def __init__(self, dim, mlp_mult, low_rank_rank=0):
        super().__init__()
        hidden = int(mlp_mult * dim)
        self.hidden = hidden
        self.low_rank_rank = int(low_rank_rank)
        if self.low_rank_rank > 0:
            if self.low_rank_rank >= min(dim, hidden):
                raise ValueError(f'LOW_RANK_MLP_RANK={self.low_rank_rank} must be smaller than min(dim, hidden)={min(dim, hidden)}')
            self.fc = FactorizedLinear(dim, hidden, self.low_rank_rank, bias=False)
            self.proj = FactorizedLinear(hidden, dim, self.low_rank_rank, bias=False, zero_init=True)
        else:
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, train_seq_len, layer_idx=0, ln_scale=False, low_rank_mlp_rank=0):
        super().__init__()
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, train_seq_len)
        self.mlp = MLP(dim, mlp_mult, low_rank_rank=low_rank_mlp_rank)
...
class GPT(nn.Module):
    ...
    def __init__(self, h):
        ...
        self.blocks = nn.ModuleList([Block(h.model_dim, h.num_heads, h.num_kv_heads, h.mlp_mult, h.rope_base, h.qk_gain_init, h.train_seq_len, layer_idx=i, ln_scale=h.ln_scale, low_rank_mlp_rank=h.low_rank_mlp_rank) for i in range(h.num_layers)])
        if h.rope_dims > 0:
        ...
        self.max_num_loops = max(0, h.num_loops)
        self.loop_start = h.loop_start
        self.loop_end = h.loop_end
        self.active_num_loops = 0
        self.encoder_indices = list(range(self.num_encoder_layers))
        self.decoder_indices = list(range(self.num_encoder_layers, h.num_layers))
        if self.max_num_loops > 0:
            self.set_active_loops(0)
    ...
    def _build_loop_indices(self, num_loops):
        loop_seg = list(range(self.loop_start, self.loop_end + 1))
        all_indices = list(range(self.loop_start))
        for _ in range(num_loops + 1):
            all_indices.extend(loop_seg)
        all_indices.extend(range(self.loop_end + 1, len(self.blocks)))
        num_enc = len(all_indices) // 2
        return (all_indices[:num_enc], all_indices[num_enc:])

    def set_active_loops(self, num_loops):
        num_loops = max(0, min(int(num_loops), self.max_num_loops))
        self.active_num_loops = num_loops
        if num_loops > 0:
            (self.encoder_indices, self.decoder_indices) = self._build_loop_indices(num_loops)
        else:
            self.encoder_indices = list(range(self.num_encoder_layers))
            self.decoder_indices = list(range(self.num_encoder_layers, self.num_encoder_layers + self.num_decoder_layers))
...
def eval_val_sliding(h, device, val_data, base_model, batch_seqs=32):
    base_model.eval()
    logits_fn = get_compiled_logits_fn(base_model)
    seq_len = h.eval_seq_len
...
def eval_val_ttt(h, device, val_data, base_model, batch_seqs=32):
    ...
    compiled_logits = get_compiled_logits_fn(base_model)
    loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    token_count = torch.zeros((), device=device, dtype=torch.float64)
    byte_count = torch.zeros((), device=device, dtype=torch.float64)
...
def eval_val_sliding_etlb(h, device, val_data, base_model):
    raise NotImplementedError('ETLB path is not implemented in this trainer. Keep ETLB_ENABLED=0.')
...
def get_compiled_logits_fn(model):
    compiled = getattr(model, '_compiled_forward_logits', None)
    if compiled is None:
        compiled = torch.compile(model.forward_logits, dynamic=False, fullgraph=True)
        model._compiled_forward_logits = compiled
    return compiled
...
def train_model(h, device, val_data):
    ...
    def desired_active_loops(frac):
        if h.num_loops <= 0:
            return 0
        if h.progressive_looping_enabled:
            if frac >= h.loop_stage2_at:
                return h.num_loops
            if frac >= h.loop_stage1_at:
                return min(h.loop_stage1_loops, h.num_loops)
            return 0
        return h.num_loops if frac >= h.enable_looping_at else 0
    ...
    if h.warmup_steps > 0:
        ...
        if h.num_loops > 0:
            base_model.set_active_loops(h.num_loops)
            log(f'loop_warmup:enabled loops:{base_model.active_num_loops} encoder:{base_model.encoder_indices} decoder:{base_model.decoder_indices}')
            for warmup_step in range(h.warmup_steps):
                step_fn(warmup_step, 1.0)
                if warmup_step <= 5 or (warmup_step + 1) % 10 == 0 or warmup_step + 1 == h.warmup_steps:
                    log(f'loop_warmup_step: {warmup_step + 1}/{h.warmup_steps}')
            base_model.set_active_loops(0)
    ...
    while True:
        ...
        scale = lr_mul(frac)
        target_loops = desired_active_loops(frac)
        if h.num_loops > 0 and target_loops != base_model.active_num_loops:
            base_model.set_active_loops(target_loops)
            log(f'layer_loop:enabled step:{step} frac:{frac:.3f} loops:{base_model.active_num_loops} encoder:{base_model.encoder_indices} decoder:{base_model.decoder_indices}')
...
def train_and_eval(h, device):
    ...
    if h.num_loops > 0:
        eval_model.set_active_loops(h.num_loops)
    ...
    if h.ttt_enabled and h.sliding_window_enabled:
        ...
        if h.num_loops > 0:
            ttt_model.set_active_loops(h.num_loops)
